...
> - 실행 기록: [G1.4 실행 명세 v8](../../evidence/runnable/g1.4/run-manifest-v8.json)

> 소속 챕터: [안전한 C로 데이터와 메모리 다루기](README.md) · 관리 코드: G1.4

## 시간과 자료

32–44시간입니다. 상태 머신·CRC 자료 5–6시간, 두 파서와 기준 인코더 8–10시간, 차이 비교·잘림 검사 6–8시간, 퍼징·결함 주입 6–8시간, TLV 전이와 기록 7–12시간으로 나눕니다. N1570 6.5.6, 6.5.7, 7.20.1과 [libFuzzer](https://llvm.org/docs/LibFuzzer.html) 문서의 퍼징 대상(Fuzz Target), 입력 모음(Corpus), 실행 선택 사항(Options)을 읽습니다.
...
libFuzzer 구동 함수와 초기 입력 모음을 작성합니다. 잘린 입력, 시작 표식 반복, 잘못된 길이, 잘못된 CRC, 여분 바이트를 포함합니다. 결정적 검사는 최대 64바이트, 고정 초기값, 100,000회, 각 실행 전 상태 초기화로 고정합니다.

## 전이 과제

처음 보는 TLV 형식에 최대 중첩 깊이 2와 전체 해석 크기 64를 적용합니다. 자원 고갈 조건을 시험합니다.

## 판정 기준

- 최대 프레임의 길이 0–19 잘림과 20바이트 완성본에서 충돌·상태 손상 없음
- 받아들인 프레임은 기준 인코더 왕복 통과
- 거부된 프레임 뒤 다음 유효 프레임 해석 가능
- 길이 선검사 제거, CRC 범위 축소, 오류 뒤 출력 쓰기, 소비 길이 한 칸 오류, 시작 표식 재사용 제거 결함을 모두 검출
- 검사 범위와 동등한 결함 주입, 남은 사각지대 기록

## 힌트

1. 길이 검증과 버퍼 위치 계산 순서를 확인합니다.
2. 오류 상태에서 소비한 바이트 수를 계약에 넣습니다.
3. CRC 시험은 파서 로직과 독립된 기준 구현을 사용합니다.
