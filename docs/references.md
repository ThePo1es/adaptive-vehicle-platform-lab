...
Gate 시작 이슈에서 실제로 읽을 문서와 절을 고정합니다. 버전, 판, commit, 접근 상태와 확인 날짜를 함께 적습니다.
...
| 판 / release / commit | |
| 확인한 절 / 출처 / 시험 | |
| 접근 상태 | Available / Limited / Unavailable |
| 사용 목적 | 어떤 설계·시험 결정을 확인할지 |
| 확인 날짜 | YYYY-MM-DD |
| 로컬 근거 | 실험·보고서 link |
| 확신 수준 | Confirmed / Partial / Unverified |
```

유료 규격에 접근할 수 없으면 공개 공식 설명, Linux kernel 문서, 공개 구현체의 source와 상호 운용 시험을 사용합니다. 규범 원문을 직접 확인한 범위는 자료 명세에 표시합니다.
...
재현용 구현 기준선은 진행 중인 Gate가 끝날 때까지 유지합니다. 매년 1월에는 AUTOSAR, Buildroot, kernel, compiler, CommonAPI·vsomeip의 현재 릴리스를 확인하고 `docs/adr/`에 차이만 기록합니다. 추가·삭제·변경된 책임, 보안 수정, 도구 호환성, 기존 시험 영향이 필수 항목입니다. 기준선을 올릴 때는 새 릴리스의 빈 환경 build와 핵심 계약 시험이 통과한 commit을 별도로 남깁니다.
...
| G2 | [C++ working draft](https://eel.is/c%2B%2Bdraft/) | `[basic.life]`, `[basic.stc]`, `[class.copy.ctor]`, `[class.dtor]`, `[unique.ptr]`, `[atomics.order]`, `[intro.races]` | 소유권/view, RAII, race 실험 |
...
| G3 | [LLVM LangRef](https://llvm.org/docs/LangRef.html) | Data Layout, Global Variables, Functions, Poison Values, Undefined Values, Memory Model | 원본 코드→IR contract 분석 |
...
기본 hardware·RTOS 조합은 [ADR-0001](adr/0001-mcu-rtos-baseline.md)을 따릅니다. vendor 문서의 revision과 silicon/board revision은 G0.2에서 실제 장비와 대조해 자료 명세에 고정합니다.

| Gate | 자료 | 확인할 내용 | 연결할 실습 |
| --- | --- | --- | --- |
| G4 | [STM32G4 RM0440](https://www.st.com/resource/en/reference_manual/rm0440-stm32g4-series-advanced-armbased-32bit-mcus-stmicroelectronics.pdf), [NUCLEO-G474RE product page](https://www.st.com/en/evaluation-tools/nucleo-g474re.html), board schematic·errata | memory/flash, RCC, interrupt, fault, DMA, watchdog, reset와 실제 board revision | 4.1–4.6 board runtime |
| G4 | Arm Cortex-M4 Devices Generic User Guide `DUI 0553B`, Armv7-M Architecture Reference Manual | programmer's model, exception entry/return, NVIC, SCB, SysTick | vector, priority, fault frame, counter |
| G4–G6 | [Zephyr NUCLEO-G474RE v4.4.0](https://docs.zephyrproject.org/4.4.0/boards/st/nucleo_g474re/doc/index.html), [Zephyr v4.4.0 코드](https://github.com/zephyrproject-rtos/zephyr/tree/v4.4.0) | board target, west manifest, kernel scheduler, 큐, timer, CAN API와 실제 설정·출처 | 4.5, 5.3–5.7, 6.1–6.2 |
| G5 | Joseph·Pandya, [*Finding Response Times in a Real-Time System*](https://doi.org/10.1093/comjnl/29.5.390), pp. 390–395; Tindell·Burns·Wellings, [*An extendible approach for analyzing fixed priority hard real-time tasks*](https://doi.org/10.1007/BF01088593), pp. 133–151; Zephyr v4.4.0 scheduling·mutex·thread analyzer·timing docs | recurrence, priority convention, blocking, release jitter, interrupt interference, 실제 mutex semantics | task model, calculator, inversion, measurement |
| G6 | [Linux SocketCAN](https://docs.kernel.org/networking/can.html), [Linux ISO-TP](https://docs.kernel.org/networking/iso15765-2.html), [Zephyr CAN controller](https://docs.zephyrproject.org/4.4.0/hardware/peripherals/can/controller.html) | Classic/FD object, DLC code/length, controller 상태, addressing, timers, Linux implementation limits | 6.1–6.8 packet·differential tests |
| G6 | Bosch CAN FD specification; ISO 11898-1:2024 edition 3, ISO 15765-2:2024 edition 4, ISO 14229-1:2026 edition 4 | arbitration/data phase, error confinement, transport timer, session/service/NRC semantics | timing, physical bench, ISO-TP, UDS |
| G7 | [AUTOSAR Classic R25-11](https://www.autosar.org/standards/classic-platform/)의 `AUTOSAR_CP_SWS_OS`, `RTE`, `CANDriver`, `CANInterface`, `PDURouter`, `COM`, `CANTransportLayer`, `DiagnosticCommunicationManager`, `DiagnosticEventManager`, `NVRAMManager`, `ECUStateManager`, `BSWModeManager`, `WatchdogDriver`, `WatchdogInterface`, `WatchdogManager`, `CommunicationManager`, `CANStateManager`, `CANNetworkManagement`, `E2ELibrary`, `SecureOnboardCommunication`, `CryptoServiceManager`, `CryptoInterface` | module 책임, interface 방향, 상태/error 책임, 생성 configuration | 7.1–7.6 개념 매핑과 설계 시험 |

AUTOSAR와 ISO 원문을 직접 확인하지 못한 항목은 로컬 동작 시험과 구분해 `Provisional` 또는 `Unverified`로 둡니다. Linux·Zephyr 구현체는 비교 기준으로 사용하며 규격 적합성 판정에는 쓰지 않습니다.

## G8–G11A 시작 자료

Linux/Adaptive 갈래는 아래 자료에서 시작합니다. AUTOSAR 문서는 R25-11 document ID와 읽은 절을 각 Sprint의 표준 자료 장부에 남깁니다.
...
G10은 로컬 concept-aligned runtime을 완성합니다. 아래 항목은 별도 연결 실습에서 실제 도구·공개 구현체와 잇습니다.

- COVESA CommonAPI의 pinned code generator·runtime·SOME/IP binding 조합으로 `.fidl/.fdepl → generated code → 애플리케이션 → packet` 경로 재현
- [Yocto Project documentation](https://docs.yoctoproject.org/)으로 P01–P03 recipe, image, SDK, license manifest 이식
- [COVESA DLT daemon](https://github.com/COVESA/dlt-daemon)과 로컬 structured event schema의 adapter·loss policy 비교
...
이 연결 실습을 수행하지 않은 release는 `Adaptive concept-aligned prototype`으로 유지합니다.
...
각 Gate는 release 하나를 고정합니다. 관련 module·functional cluster의 책임, interface, 상태, error handling을 로컬 mapping에 연결합니다. AUTOSAR PDF는 링크로 참조하고 저장소에는 복제하지 않습니다.

## Linux, SOME/IP, platform 도구
...
vsomeip, CommonAPI, DLT의 실제 역할과 로컬 Adaptive mapping을 별도 표에 기록합니다.
