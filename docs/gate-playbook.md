...
| Sprint | 주제 | 안내 실습 | 독립·전이 과제 | 판정 기준 / 종료 근거 |
| --- | --- | --- | --- | --- |
| 2.1 안전한 데이터 뷰 | RAII, 이동, 데이터 수명 | 원본 저장 공간을 함께 소유하는 데이터 뷰 | 처음 보는 수명 오류 수리 | 소유 관계, ASan, 입력 A·B |
| 2.2 힙 없는 이벤트 처리 | 고정 용량, 콜백 목록 사본 | 이벤트 처리기 | 타이머 휠 또는 라우터 | 메모리 할당 검사, 결함 주입본 4개 |
| 2.3 종료 가능한 큐 | 뮤텍스, 조건 변수, 종료 | 생산자 2·소비자 1 작업 큐 | 처음 보는 종료 경합 | 고정 깨움, 입력 100개, 스레드 종료 |
