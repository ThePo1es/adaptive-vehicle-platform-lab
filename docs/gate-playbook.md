...
| Sprint | 주제 | 안내 실습 | 독립·전이 과제 | 판정 기준 / 종료 근거 |
| --- | --- | --- | --- | --- |
| 2.1 빌린 데이터 | RAII, 이동, 데이터 수명 | 원본 저장 공간을 함께 소유하는 데이터 뷰 | 처음 보는 수명 오류 수리 | 소유 관계, ASan, 입력 A·B |
| 2.2 힙 없는 이벤트 처리 | 고정 용량, 콜백 목록 사본 | 이벤트 처리기 | 타이머 휠 또는 라우터 | 메모리 할당 검사, 결함 주입본 3개 |
| 2.3 종료 queue | mutex, condition, close | 생산자 2·소비자 1 queue | unfamiliar shutdown race | 고정 wake, 100 seeds, join |
| 2.4 다형성과 C 경계 | 가상 함수·정적 호출·함수 테이블 | C17 불투명 핸들 API | 핸들 래퍼 비교 ADR | 동일 동작 검사, ELF 보고서 |

MVP에서는 P01과 P02에 필요한 실행 기반까지만 만듭니다.
